...
window.addEventListener("load", function () {
    ...
    document.getElementById("filtrare").onclick = function () {
        ...
        // Selector multiplu pret
        var elemente = document.getElementsByName("gr_rad3")
        let inpMultiplu = [];
        for (let rad of elemente) {
            if (rad.selected) {
                inpMultiplu.push(rad.innerHTML);
            }
        }
        console.log(inpMultiplu);

        let minPret = [], maxPret = []
        for (let elem of inpMultiplu) {
            let vCal = elem.split("-")
            minPret.push(parseInt(vCal[0]))
            maxPret.push(parseInt(vCal[1]))
        }
        console.log(minPret);
        console.log(maxPret);
        ...
        // var inpDatalist = document.getElementById("inp-datalist").value;

        // preluare dater din BD si verificare conditii
        var produse = document.getElementsByClassName("produs");
        for (let produs of produse) {

            // let valDatalist = produs.getElementsByClassName("val-categorie")[0].innerHTML
            // let cond7 = (inpDatalist == valDatalist || inpDatalist == "toate")
            ...
            let valPret2 = parseFloat(produs.getElementsByClassName("val-pret")[0].innerHTML)
            
            let val3 = true;
            let val4 = true;
            for (let pret of minPret) {
                if (valPret2 > pret) {
                    val3 = true;
                    break;
                }
